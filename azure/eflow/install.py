...
with open(f"/etc/aziot/secrets/{registration_id}.pem", "w") as f:
    f.write(provisioning_details["full_chain"])

with open(f"/etc/aziot/secrets/{registration_id}.key.pem", "w") as f:
    f.write(provisioning_details["private_key"])

with open(f"/etc/aziot/secrets/aziot/identityd/dps-additional-data.json", "w") as f:
